...
class AIPSheets{
  ...
  try{
    ...
    var now = new Date();
    //Make a mock time for testing
    //var now = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 9, 55, 0, 0)
    ...
    //If time is incorrect, create a new trigger to let the program loop.
      ...
      if(min < 10){
        console.log("Incorrect time - create new trigger for "+ hour + ":0" + min);
      }
      else{
    ...
    MailApp.sendEmail("jmessina@stcharlessd.org", "GAS Error", "An error has occured in the Request Highlighter. You need to look at it!")
    deleteTimers()
    now = new Date();
    var min = Math.floor(now.getMinutes() + 5);
    var hour = Math.floor(now.getHours());
    ...
    while(hour < 2){
      ScriptApp.newTrigger("requestHighlighter").timeBased().atHour(hour).nearMinute(min).everyDays(1).create();
    }
  }
}