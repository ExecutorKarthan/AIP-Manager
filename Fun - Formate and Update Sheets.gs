function formatAndUpdateSheets(weekSheet, sourceSheet, listOfEditors, studentArray) {
  class Student{
...
for(var tab = 1; tab < numOfSheets; tab ++){
    ...
    //Assumes sheet has preexisting data in it to process
      ...
      var targetRangeData = weekSheet[tab].getRange(1, 1, tabLastRow, 10).getValues();
  ...
  //Create a map of the target data, making it an ID
      ...
      for(var arrayValue = 0; arrayValue < studentArray.length; arrayValue++){
        ...
        studentArray[arrayValue].notes = targetRangeData[correspondingRow][9]; 
        }
      ...
      //Copy over all student data
      for(var objNum = 0; objNum < studentArray.length; objNum++){
        var changeArray = [[studentArray[objNum].name, studentArray[objNum].grade, studentArray[objNum].aipTeacher, studentArray[objNum].firstTeacher, studentArray[objNum].secondTeacher, studentArray[objNum].thirdTeacher, studentArray[objNum].otherTeacher, false, false, studentArray[objNum].notes]];
        var rowToFill = weekSheet[tab].getRange(4+objNum, 1, 1, 10);
        rowToFill.setValues(changeArray);
      }
